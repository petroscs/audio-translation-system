...
- **Web translator (browser):** The web translator is served over **HTTPS** on port 3002 and proxies `/api` and `/ws` through nginx to the backend. This avoids CORS and mixed-content issues and does not require an `API_URL` build-time setting.
